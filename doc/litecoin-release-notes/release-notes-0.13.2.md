Tooncoin Core version 0.13.2 is now available from:

  <https://download.tooncoin.org/tooncoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/tooncoin-project/tooncoin/issues>
...
updates will be released anymore. Without security updates, using a tooncoin
...
ECDSA signatures inside Tooncoin transactions now use validation using
...
Previous versions of Tooncoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Tooncoin Core was to raise the default minimum
relay fee.

Tooncoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Tooncoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running tooncoind without having to do any manual configuration.
...
Tooncoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Tooncoin Core will relay transactions with
...
entirely in Tooncoin Core 0.13, and it is at this time undecided whether the
...
Tooncoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Tooncoin Core automatically creates a hidden service to listen on, without
manual configuration. Tooncoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Tooncoin Core is listening, and
...
Tooncoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Tooncoin
Core to use floating fees. By default, Tooncoin Core will use floating
fees.
...
(default: `0.0002` TOON/kB).

At all times, Tooncoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) TOON.
Furthermore, Tooncoin Core will never create transactions paying less than
...
`tooncoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /tooncoinrpc>
        ...
        # optional bypass tooncoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< tooncoinrpc:<password>
    </Location>
...
Restarting tooncoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in tooncoin.conf
...
tooncoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/tooncoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Tooncoin Core code base has
...
- `tooncoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `tooncoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Tooncoin scripting system that in combination with
...
Tooncoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Tooncoin Core.
...
The two phases are distinct in the Tooncoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Tooncoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Tooncoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Tooncoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of tooncoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of tooncoins being spent is exactly
...
- **Increased security for multisig:** Tooncoin addresses (both P2PKH addresses
  ...
  believed to be provided by Tooncoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Tooncoin's current ledger (called the UTXO set),
  ...
  note that Tooncoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Tooncoin users to individually opt-in to almost any change in the Tooncoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Tooncoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Tooncoin
client to Tooncoin Core 0.13.2 to signal for segwit support. When a
...
Since Tooncoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `tooncoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `tooncoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
