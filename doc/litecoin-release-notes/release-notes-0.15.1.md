Tooncoin Core version *0.15.1* is now available from:

  <https://download.tooncoin.org/tooncoin-0.15.1/>
...
A number of changes to the way Tooncoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 toons per kB (0.00001 TOON). This is to prepare for dropping the minimum transaction fee to 0.1 toons per kB (0.0001 TOON) in the next release.
